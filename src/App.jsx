    ...
    <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl scale-100 animate-in zoom-in-95 duration-200">
        <div className="bg-gradient-to-br from-amber-300 via-orange-400 to-rose-500 p-6 text-white text-center relative overflow-hidden">
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 bg-black/20 hover:bg-black/30 p-1 rounded-full transition-colors"
          >
            <X size={20} />
          </button>
        ...
        <div className="p-6 space-y-4">
          ...
          <button
            onClick={onUpgrade}
            className="w-full py-4 mt-4 bg-gradient-to-r from-amber-400 to-orange-500 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-transform"
          >
            Upgrade for $9.99/mo
          </button>
        </div>
...
function AuthScreen({ onLogin, onSignup, loading }) {
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center p-6 bg-gradient-to-br from-rose-500 to-orange-500 text-white overflow-y-auto">
      ...
      <p className="mb-6 text-white/80 font-medium text-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-rose-500 font-medium"
              required
            />
          </div>
          <div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gray-900 text-white font-bold py-3.5 rounded-xl mt-2 active:scale-95 transition-transform disabled:opacity-70 flex justify-center"
          >
            {loading ? (
...
function DiscoverScreen({ profiles, onSwipe, onTriggerPremium }) {
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center animate-in fade-in">
        ...
        <p className="text-gray-500 mb-8 max-w-[200px]">
          We couldn&apos;t find any users nearby.
        </p>
      </div>
    );
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center animate-in fade-in">
        ...
        <button
          ...
          className="px-6 py-3 bg-white border-2 border-gray-200 rounded-full font-bold text-gray-600"
        >
          Review Again
        </button>
      </div>
  ...
  const handleSwipeLocal = (direction) => {
  ...
  return (
    <div className="h-full relative overflow-hidden bg-gray-50">
      ...
      <div className="absolute inset-3 top-2 bottom-6 bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden z-10 flex flex-col animate-in zoom-in-95 duration-300">
        <div className="h-[60%] bg-gray-200 relative group">
          ...
          <div className="absolute top-4 left-4 bg-white/90 backdrop-blur px-3 py-1.5 rounded-full text-xs font-bold text-gray-700 shadow-sm border border-white/50">
            {currentProfile.distance || '5'} miles away
          </div>
          {currentProfile.gym && (
          ...
          <div className="flex justify-between items-start mb-2">
            <div>
              ...
              <div className="text-rose-500 font-bold text-xs uppercase tracking-wide mt-1">
                {currentProfile.intent || 'Gym Partner'}
              </div>
            </div>
          </div>
      ...
      <div className="absolute bottom-8 left-0 right-0 z-20 flex justify-center items-center gap-6">
        ...
        <button
          onClick={() => handleSwipeLocal('right')}
...
function ChatScreen({ match, onBack, currentUser }) {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: 'Hey! I saw you train at Equinox too.',
      senderId: 'them',
      timestamp: new Date(),
    },
  ]);
